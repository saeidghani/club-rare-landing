...
const useAllStakedValue = () => {
  ...
  const fetchAllStakedValue = useCallback(async () => {
    ...
    await Promise.all(pids.map(async pid => {
      ...
      if ([config.usdt, config.usdc].includes(token1)) usdValue = totalToken2Value;
      else if (token1 === config.eth) { // AGOV- ETH , ANSWER -ETH  , USDC-ETH?
        ...
        const ethUsd = reserves['_reserve1'] / reserves['_reserve0'] * (10 ** 12); // ETH in USD Price

        usdValue =  ethUsd *totalToken2Value;    // AGOV-ETH  28 , ANSWER-ETH 7 ,  ETHUSD 476 1ETH= 476$
      } 
        ...
        const ethUsd = reserves['_reserve1'] / reserves['_reserve0'] * (10 ** 12);
        const usd = tokenEth2 * ethUsd;
        if (token0 === config.seal){
          usdValue = 444  ; // Locked Value(AGOV) * AGOV in USD Price 
        }else{
        usdValue =  totalToken2Value *usd  ; // Locked Value(AGOV) * AGOV in USD Price 
        }
  
      }  
      else if (token1 === config.seal) {  //AGOV-SEAL
        ...
        usdValue = 555 ; //totalToken2Value *usd  ; // Locked Value(AGOV) * AGOV in USD Price 
