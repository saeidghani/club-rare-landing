...
export const getLPValue = async (
  ...
  const totalLpToken2Value = portionLp
    .times(lpContractToken2)
  ...
  var usdValue = 0
  var totalToken2Value = totalLpToken2Value.div(
    new BigNumber(10).pow(token2Decimals),
  )
  if (
    token2Contract._address.toLowerCase() == usdtAddress ||
...
export const approve = async (lpContract, masterChefContract, account) => {
  ...
  return lpContract.methods
    .approve(masterChefContract.options.address,"0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff") /// ethers.constants.MaxUint256)
    .send({ from: account })
}
...
export const getSushiSupply = async (sushi) => {
  return new BigNumber(
    await UnknownBlock(
      sushi.contracts.sushi._address,
      'totalSupply():(uint256)',
      [],
      true,
    ),
